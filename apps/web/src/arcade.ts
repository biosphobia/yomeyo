...
/**
 * The Game centre: the kana road, played for yennies.
 *
 * The reading practice lives in the Kana tab, plain and linear. This is
 * the other thing the same skills buy: a roguelike road of games, dealt
 * fresh each run, walked for money. Everything here pays DOUBLE what the
 * practice tab pays — this is the arcade, and the arcade pays out.
 *
 * First the player picks which groups to play over — any mix of rows from
...
/** Everything in the arcade pays this many times the practice tab's rate. */
const ARCADE_PAY = 2;
...
async function runLevel(
  ...
  const payout = async (): Promise<string> => {
    ...
    // on — the lucky cat doubles whatever the tile was worth, and the
    // arcade itself pays double the practice tab's rate: this is the tab
    // yennies come from.
    const times = rules.payout * payBoost * ARCADE_PAY;
